...
function chooseCard(e) {
	...
	let counter = localStorage.getItem("counter") || 0;
	if (e.target.parentElement.id == correctCard) {
		counter++;
	} else {
		counter = 0;
	}
	...
	if (correctCardElement.classList.contains("rotate90")) {
		correctCardElement.style.transform = "scale(1.3) rotate(90deg)"; //TODO fix this, is not working, if the winning card is a double card, it isn't rotated once highlighted
		correctCardElement.style.transform = "scale(1.3)";
	} else {
		correctCardElement.style.transform = "scale(1.3)";
	}
